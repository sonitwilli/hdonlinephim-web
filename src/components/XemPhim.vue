...
export default {
  ...
  data () {
      return{
        playerOptions: {
          ...
          // sources: [{
          //   type: "video/mp4",
          //   src: "https://content-na.drive.amazonaws.com/cdproxy/templink/NSKyAMiZs7T9rZOFascsRPu0YuCpTyaj7qIKQfPbWUgeJxFPc?path=%2Falt%2Fvideo&y=720"
          // }],
          // sources: this.data_play,
  ...
  methods: {
    getFilms(id) {
      this.$http.get(this.api_url+'api/film/'+id).then(response => {
        if(response.body.status === 200){
          this.film = response.body.data;
          this.playerOptions.poster = this.film.background_link;
          this.playerOptions.sources = response.body.data_play;
          this.playerOptions.autoplay = 'true';
          console.log('get Data Done');
        }
      }, response => {
        console.log('error get film detail')
      });
    },
    // listen event
    onPlayerPlay(player) {
